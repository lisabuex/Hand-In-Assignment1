...
author: "Anastasia Suckau (440938)
         Lisa Büx (441574)
...
FEVplot <- ggplot(lungcap, aes(FEV)) + 
           ...
           ggtitle("Density Plot with Mean and Median Line ") +
...
> Based on the t-statistic a p-value can be calculated. The p-value is the probability of achieving a |t| as large as or larger than the observed absolute t value if the null hypothesis was true, where \(H_0\) is \(\hat_{\beta_i}=0\):
...
> The predicted probability by the logistic regression of player one winning the game is equal to 38.29%. That means the model predicts an outcome of y = 0 which means that player one will lose the game. The actual result of the game was that player one lost. That means the model successfully predicted the actual outcome. 
...
> If we use the adjusted R-squared value as our decision criterion the best subset would consist of 8 predictors as it can be seen in the plot. The djusted R-squared for the best subset is equal to 0.9781214. That means the best subset can explain 97.81% of the variance in the response variable `log_price`.  
...
> The adjusted `R^2` is a commonly known tool when it comes to evaluating how well a model fits the actual data. 

> Formally expressed the `R^2` for a least squares model with d variables is the following: 
$$\text{Adjusted R}^2=1-\frac{RSS/(n-d-1)}{TSS/(n-1)}$$

> A high adjusted `R_2` means that the test error is small. This is different for alternative approaches such as `C_p`, `AIC` and `BIC`, where a small error would be indicated by a small value. 

> The cross-validation-approach calculates an error for each model and selects the `k` with the smallest estimated test error. One advantage of this approach towards the adjusted `R^2` is that an estimate of the error variance here is not required. Furthermore it makes few assumptions about the true model and can be used in complex cases where the degrees of freedom cannot be identified. On the other hand, the adjusted `R^2` is commonly known among most scientists and is easy to interpret. 
